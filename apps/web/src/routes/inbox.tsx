...
export function InboxPage() {
  const [rows, setRows] = React.useState<Notification[] | null>(null);

  const load = React.useCallback(() => {
    api.get<Notification[]>('/api/notifications').then(setRows);
  }, []);
    ...
    <div>
      ...
      <div className="rounded-xl border border-hairline bg-surface">
        {rows?.length ? (
          ...
          <p className="p-4 text-body-sm text-ink-3">Nothing yet.</p>
        )}
      </div>
    </div>
  );

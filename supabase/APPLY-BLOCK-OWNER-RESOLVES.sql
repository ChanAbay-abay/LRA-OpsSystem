-- =====================================================================
-- LRA Ops :: APPLY-BLOCK-OWNER-RESOLVES.sql
--
-- Ready-to-paste SQL for the Supabase SQL editor. Same pattern as
-- APPLY-TO-PRODUCTION.sql, APPLY-PHASE-3-4-5.sql and
-- APPLY-READONLY-AND-CATALOG-POINTS.sql: no agent in this project has
-- ever held DDL credentials, so Chan applies migrations by pasting them.
--
-- It contains exactly one migration:
--   1. 20260910190000_ops_task_block_owner_resolves.sql
--
-- WHY IT MATTERS TONIGHT. This is the database half of Chan's report
-- "users cant unblock a task, fix it". The client half is already
-- deployed in the web app, and it MIRRORS the policy below -- so until
-- this runs, a task's owner is offered a Resolve button that PostgREST
-- answers with a 422. Applying it is what makes the button work.
--
-- WHAT IT DOES NOT DO: it drops nothing, touches no data, touches no
-- read path, and touches neither `auth` nor any other schema. It
-- replaces one UPDATE policy with the same policy plus one additional
-- OR branch. It is strictly a widening of who may resolve a block, and
-- the read-only / deactivated-account guards are preserved verbatim.
--
-- NOT YET EXECUTED ANYWHERE. Written by reading the live policy
-- definition it replaces; reviewed, but it has not met a database.
-- Afterwards, run `npm run test:rls` -- nine assertions were added to
-- `supabase/tests/rls_test.sql` for exactly this policy and they have
-- never run either.
-- =====================================================================

-- =====================================================================
-- LRA Ops :: APPLY-BULK-EDIT-BATCHES.sql
--
-- Ready-to-paste SQL for the Supabase SQL editor. Same pattern as
-- APPLY-TO-PRODUCTION.sql, APPLY-PHASE-3-4-5.sql,
-- APPLY-READONLY-AND-CATALOG-POINTS.sql and
-- APPLY-BLOCK-OWNER-RESOLVES.sql: no agent in this project has ever held
-- DDL credentials, so Chan applies migrations by pasting them.
--
-- It contains exactly one migration:
--   1. 20260910200000_ops_task_edit_batches.sql
--
-- WHAT IT IS. The database half of Chan's 2026-09-10 ask: "GM can send a
-- request to edit (should be done by bulk like an edit feature on google
-- docs), then approve by admin or founder showing what changed like
-- before and after". It adds `ops.task_edit_batches` (one suggestion
-- summary owning N ordinary `ops.task_edit_requests` children), three
-- functions (`create_edit_batch`, `decide_edit_batch`,
-- `withdraw_edit_batch`), and it WIDENS who may approve an edit request
-- from `core.is_clearing_founder()` to `core.is_founder() and not
-- core.is_read_only()` -- founder or admin, never a read-only account.
--
-- THE ONE BEHAVIOUR CHANGE TO EXISTING DATA PATHS, stated plainly: after
-- this runs, a NON-CLEARING founder can approve a single task edit
-- request, which they could not before. That is Chan's instruction ("by
-- admin or founder", no clearing qualifier) and it supersedes the
-- reasoning recorded in 20260910140000's header. ERC and DCA still
-- cannot: they are read-only, and the read-only refusal now sits both at
-- the first statement of the function AND inside the decider predicate.
--
-- WHAT IT DOES NOT DO. It drops nothing. It rewrites no existing data.
-- It touches no read path, no other schema (`auth`, `core` tables are
-- read but not altered), and no existing column except by ADDING one
-- nullable `batch_id` to `ops.task_edit_requests` -- where NULL means
-- exactly today's single-request behaviour, so every existing route,
-- policy and test stays valid. The only existing function it replaces is
-- `ops.enforce_task_edit_request_transition()`, reproduced in full from
-- its current live body (20260910170000's copy) with three marked
-- changes.
--
-- NOT YET EXECUTED ANYWHERE. Written by reading the live function it
-- replaces; reviewed, but it has not met a database. No session in this
-- repo can run DDL, and the RLS suite needs a DATABASE_URL password that
-- is not in the repo -- so the ~40 assertions added to
-- `supabase/tests/rls_test.sql` for this feature HAVE NEVER RUN either.
-- Afterwards, run `npm run test:rls`: it opens a transaction and rolls
-- it back, so it is safe against the live project, and its verdict line
-- is the first real evidence any of this works.
-- =====================================================================

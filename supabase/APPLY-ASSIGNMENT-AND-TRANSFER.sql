-- =====================================================================
-- LRA Ops :: APPLY-ASSIGNMENT-AND-TRANSFER.sql
--
-- Ready-to-paste SQL for the Supabase SQL editor. Same pattern as the
-- other APPLY-*.sql files: no agent in this project has ever held DDL
-- credentials, so Chan applies migrations by pasting them.
--
-- It contains exactly one migration:
--   1. 20260911120000_ops_task_assignment_and_transfer.sql
--
-- WHAT IT IS, in one paragraph: `ops.tasks.owner_user_id` becomes
-- nullable (unassigned tasks exist and sit in the week's list until
-- someone takes them); an unassigned task cannot change status or be
-- committed (one early, unconditional refusal, so nothing downstream
-- silently mishandles a NULL comparison); any active ops member may
-- self-claim an unassigned task; GM/founder/admin may assign directly;
-- a new `ops.task_assignment_invites` table lets the current owner
-- invite someone else to take a task over, RLS-gated so only
-- `ops.accept_task_transfer()` can ever move an invite to `accepted`;
-- a committed task's promise (`committed_by_user_id`, new column) does
-- NOT move with a transfer -- only ownership does; every owner change
-- writes an audit row, so the handoff is readable later via
-- `GET /api/tasks/:id/history`; and `ops.stamp_catalog_points` now
-- re-derives `catalog_points` (and, while the week is still planning,
-- `committed_points`) when a task's catalog type changes on UPDATE, not
-- only on INSERT -- closing the hole where changing a task's type
-- silently left it priced at its old type's points.
--
-- THIS RUNS ON PRODUCTION ONLY IF `ops.admin_correct_task` ALREADY
-- EXISTS THERE (from APPLY-ADMIN-CORRECTIONS.sql) -- statement 2b below
-- reads the same `ops.admin_correction_task` / `ops.transfer_accept_task`
-- GUC pattern that migration introduced. If that has not been pasted
-- yet, paste it first.
-- =====================================================================
